...
- Composite, Decorator는 재귀적 합성 기법을 사용하기 때문에 구조가 비슷하다. 하지만 목적은 다르다.
  - Decorator 패턴은 **상속 없이 객체에 새로운 기능을 추가하려는 것**이 목적이다.
  - Composite 패턴은 클래스 구조화에 초점을 맞춘것으로 **어떻게 관련 객체들을 하나의 인터페이스로 다룰 수 있도록 일관성을 부여하는 것**이 목적이다.
  - 이런 목적들을 상충되는 것이 아닌, 상호보완적이므로 두 패턴이 함께 사용할 때가 많이 있다.
- Decorator, Proxy는 대상 객체에 대한 참조자를 관리하므로 구조가 비슷하다. 하지만 목적은 다르다.
  - Proxy는 Decorator와 달리 동적으로 어떤 기능을 추가 및 제거하지 않는다.
  - Proxy의 목적은 대상 객체에 대한 접근을 제어하는 것이 목적이다.
  - Proxy는 참조하는 Target이 실제 기능을 제공하지만 Decorator는 Decorator 자체에서도 기능적인일을 담당한다. 
