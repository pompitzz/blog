...
- 가능하면 인터페이스의 디폴트 메서드를 통해 골격 구현을 제공하고, 디폴트 메서드로만 제공이 불가능한 경우에 추상 골격 구현을 제공하자.
...
- 일반적으로 프로그램의 90%의 시간을 단 10%의 코드에서 사용한다는 사실을 기억하자.
...
- 호출자가 아규먼트에 부적절한 값을 넘길 대 사용하는 예외다.
